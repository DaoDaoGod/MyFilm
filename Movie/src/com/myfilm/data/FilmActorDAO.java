 ...
 * 
 * @see com.myfilm.data.FilmActor
 * @author MyEclipse Persistence Tools
 */
public class FilmActorDAO extends HibernateDaoSupport {
	...
	public FilmActor findById(com.myfilm.data.FilmActorId id) {
		log.debug("getting FilmActor instance with id: " + id);
		try {
			FilmActor instance = (FilmActor) getHibernateTemplate().get(
					"com.myfilm.data.FilmActor", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<FilmActor> findByExample(FilmActor instance) {
		...
		try {
			List<FilmActor> results = (List<FilmActor>) getHibernateTemplate()
	...
	public FilmActor merge(FilmActor detachedInstance) {
		...
		try {
			FilmActor result = (FilmActor) getHibernateTemplate().merge(
					detachedInstance);
